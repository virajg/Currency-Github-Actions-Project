...
import org.example.cmp.utils.DoubleConverter
import org.example.cmp.utils.GetBebasFontFamily
import org.example.cmp.utils.calculateExchangeRate
import org.example.cmp.utils.convert
            ...
            onClick = {
                if (source.isSuccess() && target.isSuccess()) {
                    val exchangeRate = calculateExchangeRate(
                        source = source.getSuccessData().value?.toDoubleOrNull()?:0.0,
                        target = target.getSuccessData().value?.toDoubleOrNull()?:0.0
                    )
